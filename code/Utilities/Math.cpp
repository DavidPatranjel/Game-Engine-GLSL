...
Camera& Camera::moveOx(int sense)
{
	Vector3 forward = xAxis * sense;
	Vector3 displacementVector = forward * moveSpeed * deltaTime;
	position += displacementVector;
	target += displacementVector;
	
	updateWorldView();
	return *this;
}

Camera& Camera::moveOy(int sense)
{
	Vector3 forward = yAxis * sense;
	Vector3 displacementVector = forward * moveSpeed * deltaTime;
	position += displacementVector;
	target += displacementVector;
...
Camera& Camera::moveOz(int sense)
{
	Vector3 forward = zAxis * sense;
	Vector3 displacementVector = forward * moveSpeed * deltaTime;
	position += displacementVector;
	target += displacementVector;
...
Camera& Camera::rotateOx(int sense)
{
	Matrix mRotateOX;
	mRotateOX.SetRotationX(sense * rotateSpeed * deltaTime);
...
Camera& Camera::rotateOy(int sense)
{
	Matrix mRotateOY;
	mRotateOY.SetRotationY(sense * rotateSpeed * deltaTime);
...
Camera& Camera::rotateOz(int sense)
{
	Matrix mRotateOZ;
	mRotateOZ.SetRotationZ(sense * rotateSpeed * deltaTime);
...
Camera& Camera::updateWorldView()
{
	...
	Tt.SetTranslation(position * (-1));

	worldMatrix = R * T;
	viewMatrix = Tt * R.Transpose();
